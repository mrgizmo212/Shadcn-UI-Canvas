...
- 'buttonGroup': horizontal grouping of buttons (props: buttons: comma separated list like 'Left, Middle, Right', activeIndex: number)
- 'inputGroup': input with text badges (props: prefix like 'https://', placeholder like 'github.com', suffix like '/repo')
- 'inputOtp': One-time password PIN grid (props: length: number (e.g. 6), value: string (e.g. '123'))
- 'breadcrumb': horizontal route selector list (props: links: comma separated list like 'Home, Settings, Keys')
- 'popover': focus option bubble trigger (props: trigger, title, description)
- 'tooltip': floating hint label (props: trigger, content)
- 'nativeSelect': Native drop chooser select box (props: label, options: comma separated list)
- 'combobox': searchable select box (props: placeholder like 'Search...', options: comma separated list, selected: string)
- 'toggle': pressable on/off button (props: label, isActive: boolean)
- 'toggleGroup': exclusive option toggles (props: options: comma separated list, selected: string)
- 'kbd': keyboard shortcut badge (props: text like '⌘K')
- 'spinner': loading animator spinner (props: size: 'sm'|'md'|'lg')
- 'typography': rich text blocks (props: variant: 'h1'|'h2'|'h3'|'p'|'lead'|'code', text: string)
- 'emptyState': placeholder state block (props: title, description, buttonLabel)
- 'fieldContainer': form field wrapper (props: label, description, helper)
- 'listItem': descriptive list layout card (props: title, description, meta like 'v1.0')
- 'datePicker': date browser chooser button (props: label, selectedDate string like '2026-05-20')
- 'direction': text direction indicator LTR/RTL (props: dir: 'ltr'|'rtl')
- 'alertDialog': critical cancel/confirm overlay modal (props: title, description, cancelLabel, confirmLabel)
- 'dataTable': structured data list displaying metrics (props: title, headers: comma separated, rows: columns divided by comma, rows piped with '|' e.g. 'ais, media, Glacier | repo, json, Standard')
- 'drawer': bottom slide-up overlay page segment (props: title, description)
- 'command': command list shortcuts popup (props: placeholder, groups: comma separated headings, items: comma separated shortcuts)
- 'contextMenu': simulated right-click pop triggers (props: trigger: trigger label text, items: comma separated choices)
- 'dropdownMenu': overlay dropdown item menus selector (props: trigger: trigger button title, items: comma separated menu choices)
- 'menubar': system master control menus bar (props: menus: comma separated menus list like 'File, Edit, Help')
- 'navigationMenu': global navigation search categories headers (props: trigger: category text, items: comma separated catalog routes)
- 'pagination': table routing page links rows (props: currentPage: number, totalPages: number)
- 'hoverCard': float informative popups on mouse hover (props: triggerText, title, description, joined: joined context info)
- 'resizable': split sliding workspace panels (props: leftLabel, rightLabel)
- 'aspectRatio': strict-fitted dimension constraints layouts for images (props: ratio: '16/9'|'4/3'|'1/1', imageSrc: url path)
- 'carousel': sliders slideshow of elements (props: slides: comma-separated captions list)
- 'chart': stats area, line or bar custom graph models (props: title, type: 'area'|'line'|'bar', dataPoints: data values formatted like 'Mon, 40 | Tue, 60 | Wed, 45')
- 'collapsible': expander options list block (props: title, content: piped attributes list)
- 'scrollArea': overflow scroll context window (props: content: piped custom elements/lines list)
- 'sidebar': control systems side menu navigation deck (props: title, items: comma-separated tabs items)
- 'sonner': custom triggering toast overlay (props: label, title, description)
- 'toast': interactive trigger warnings toaster alert blocks (props: label, title, description)
